...
/// The Varispeed Write GL passes: `write` (Send·input → buffer layer, a pure
/// overwrite) and the shared `output` (passthrough of the input — the record head
...
impl WriteShaders {
    ...
    /// must be disabled by the caller — a straight write, no read of the slot.
