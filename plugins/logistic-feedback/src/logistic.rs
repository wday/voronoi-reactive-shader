...
use crate::params::{self, LogisticParams, NUM_PARAMS};
use crate::shader::LogisticShader;

pub struct LogisticFeedback {
    params: LogisticParams,
    shader: Option<LogisticShader>,
}

impl SimpleFFGLInstance for LogisticFeedback {
    ...
    fn draw(&mut self, _data: &FFGLData, frame_data: GLInput) {
        if self.shader.is_none() {
            self.shader = Some(LogisticShader::new());
        }
    ...
    fn plugin_info() -> ffgl_core::info::PluginInfo {
        ffgl_core::info::PluginInfo {
            unique_id: *b"LgFb",
            name: *b"LogisticFeedback",
            ty: ffgl_core::info::PluginType::Effect,
            about: "Per-pixel logistic map for bifurcation and chaos".to_string(),
            description: "Nonlinear feedback dynamics with spatial modulation".to_string(),
        }
    }
}
